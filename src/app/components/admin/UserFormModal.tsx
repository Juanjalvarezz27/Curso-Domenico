...
import { Loader2, Check, CreditCard, ShieldCheck, X } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(errorData.error || "Ocurrió un error al registrar.");
      }
    } catch (error) {
      ...
      <div className="bg-white w-full max-w-lg rounded-t-[2.5rem] md:rounded-[3rem] shadow-2xl overflow-hidden relative z-10 flex flex-col max-h-[95vh] animate-in slide-in-from-bottom md:zoom-in-95 duration-200">
        ...
        <div className="shrink-0 h-24 md:h-28 bg-gray-900 flex items-center px-8 md:px-10 relative">
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute top-6 right-6 text-white/70 hover:text-white p-2 rounded-full hover:bg-gray-800 transition-colors"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-5 overflow-y-auto">
          ...
          <div className="flex flex-col md:flex-row gap-3 pt-4">
            <button
              disabled={loading}
              className="order-1 md:order-2 flex-2 bg-gray-900 text-white font-black py-5 rounded-2xl hover:bg-orange-600 transition-all active:scale-[0.98] flex items-center justify-center gap-3 uppercase text-[10px] tracking-[0.2em] shadow-xl w-full"
            >
